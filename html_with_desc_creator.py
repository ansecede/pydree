...
# %%
def rename_root(soup_body: Tag, tooltip: bool = True, *name_parts: str):
    new_name = ""
    for part in name_parts:
        new_name += part
    if not tooltip:
        try:
            soup_body.find(class_="root-tooltip").attrs["class"] = ""
        except:
            pass
...
def consice_populate_html(parent_soup: Tag, parent_stats: FSItem):
    ...
    for children_stats in parent_stats["children"]:
        if children_stats["type"] == "directory" and children_stats.get("description", None):
            directory = rtg.create_dir(children_stats["name"], children_stats.get("description", ""), None)
...
vol1_root_dir = rtg.create_dir(meta_vol1["name"], meta_vol1.get("description", ""), None)
...
# NAS Vol2
clear_root(base_report_body)
vol2_root_dir = rtg.create_dir(meta_vol2["name"], meta_vol2.get("description", ""), None)
main_container = base_report_body.nav
...
with open("reports/Descripciones_NAS.html", "w") as file:
    file.write(str(base_report_soup))
